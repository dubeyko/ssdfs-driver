...
if [[ $# -lt 3 ]]
then
    echo "Usage: $0 BLOCK-DEVICE MOUNTPOINT REPEAT_COUNT"
    exit 1
fi
    ...
    do
      ...
      do
        ...
        do
          ...
          folder_name=${block_size[$i]}"-"$peb_size"-"$seg_size"-"$k"-MOUNT-DUMP-0001"
          mkdir $folder_name
          ...
          folder_name=${block_size[$i]}"-"$peb_size"-"$seg_size"-"$k"-MOUNT-DUMP-0002"
          mkdir $folder_name
          chown -hR slavad $folder_name
          cd $folder_name
          dump.ssdfs -p parse_all,raw_dump -o ./ $1
