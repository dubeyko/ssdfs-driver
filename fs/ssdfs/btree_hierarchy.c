...
finish_prepare_level:
	...
	insert->pos.count = search->request.count;
	insert->op_state = SSDFS_BTREE_AREA_OP_REQUESTED;
...
void ssdfs_btree_cancel_insert_item(struct ssdfs_btree_level *level)
{
	...
	insert->op_state = SSDFS_BTREE_AREA_OP_UNKNOWN;
	insert->hash.start = U64_MAX;
	insert->hash.end = U64_MAX;
	insert->pos.state = SSDFS_HASH_RANGE_INTERSECTION_UNDEFINED;
	insert->pos.start = U16_MAX;
	insert->pos.count = 0;
}
				      ...
				      struct ssdfs_btree_level *child)
{
	...
	case SSDFS_BTREE_ROOT_NODE:
		...
		case SSDFS_BTREE_HYBRID_NODE:
			...
			start_hash = parent_node->index_area.start_hash;
			end_hash = parent_node->index_area.end_hash;
	...
	case SSDFS_BTREE_INDEX_NODE:
		...
		case SSDFS_BTREE_HYBRID_NODE:
			...
			parent->index_area.insert.pos.state =
					SSDFS_HASH_RANGE_LEFT_ADJACENT;
			parent->index_area.insert.hash.start = start_hash;
			parent->index_area.insert.hash.end = end_hash;
			parent->index_area.insert.pos.start = 0;
			parent->index_area.insert.pos.count = index_count;
			parent->index_area.insert.op_state =
					SSDFS_BTREE_AREA_OP_REQUESTED;
			break;
		...
		if (can_add_new_index(parent_node)) {
			...
			if (ssdfs_need_move_items_to_parent(child)) {
				err = ssdfs_prepare_move_items_to_parent(search,
									 ...
									 child);
				if (unlikely(err)) {
					SSDFS_ERR("fail to check moving "
						  "to sibling: err %d\n",
						  err);
					return err;
				}
			  ...
			  struct ssdfs_btree_level *child)
{
	...
	case SSDFS_BTREE_HYBRID_NODE:
		...
		err = ssdfs_btree_node_add_index(child_node, &key);
		if (unlikely(err))
			SSDFS_ERR("fail to add index: err %d\n", err);
		break;
 ...
 */
static
int __ssdfs_btree_update_index(struct ssdfs_btree_level *parent,
				struct ssdfs_btree_level *child,
				struct ssdfs_btree_node *parent_node,
				struct ssdfs_btree_node *child_node)
{
...
#ifdef CONFIG_SSDFS_DEBUG
	BUG_ON(!parent || !child || !parent_node || !child_node);
#endif /* CONFIG_SSDFS_DEBUG */

	SSDFS_DBG("parent %p, child %p, parent_node %p, child_node %p\n",
		  parent, child, parent_node, child_node);
	...
	if (parent_type == SSDFS_BTREE_HYBRID_NODE &&
	    ...
	    parent_node == child_node) {
		old_hash = parent->nodes.old_node.items_hash.start;
		if (old_hash == U64_MAX) {
			err = -ERANGE;
...
finish_update_index:
	...
	switch (child_type) {
	case SSDFS_BTREE_HYBRID_NODE:
		old_hash = child->nodes.old_node.items_hash.start;
		if (old_hash == U64_MAX) {
			SSDFS_WARN("invalid old hash\n");
			return -ERANGE;
		}
		...
		err = ssdfs_btree_node_change_index(child_node,
						    &old_key,
						    &new_key);
		if (unlikely(err)) {
			SSDFS_ERR("fail to update index: err %d\n",
	...
	case SSDFS_BTREE_HYBRID_NODE:
		...
		if (old_hash == start_hash) {
			...
			spin_lock(&parent_node->descriptor_lock);
			memcpy(&new_key, &parent_node->node_index,
				sizeof(struct ssdfs_btree_index_key));
			new_key.index.hash = cpu_to_le64(start_hash);
			spin_unlock(&parent_node->descriptor_lock);
			...
			err = ssdfs_btree_node_add_index(parent_node, &new_key);
			if (unlikely(err)) {
				SSDFS_ERR("fail to add index: err %d\n", err);
				return err;
			}
