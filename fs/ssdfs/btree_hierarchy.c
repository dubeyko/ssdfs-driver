				     ...
				     struct ssdfs_btree_node *node)
{
	...
	SSDFS_DBG("level %p, start_hash %llu, "
		  "end_hash %llu, node %p\n",
	...
	if (start_hash > end_hash) {
		err = -ERANGE;
		SSDFS_ERR("invalid range: start_hash %llu, end_hash %llu\n",
			  start_hash, end_hash);
		goto finish_prepare_level;
	}
				      ...
				      struct ssdfs_btree_level *child)
{
	...
	case SSDFS_BTREE_ROOT_NODE:
		...
		case SSDFS_BTREE_HYBRID_NODE:
			if (parent->index_area.free_space > 0)
				return 0;
	...
	case SSDFS_BTREE_INDEX_NODE:
		...
		case SSDFS_BTREE_HYBRID_NODE:
			if (child->index_area.free_space > 0)
				return 0;
				    ...
				    struct ssdfs_btree_level *child)
{
	...
	case SSDFS_BTREE_HYBRID_NODE:
		...
		case SSDFS_BTREE_HYBRID_NODE:
			...
			max_item_size = parent_node->items_area.max_item_size;
			items_count = parent_node->items_area.items_count;
			up_read(&parent_node->header_lock);

			if (free_space >= max_item_size) {
				SSDFS_WARN("items_area.free_space %u, "
					   "items_area.max_item_size %u\n",
					   free_space, max_item_size);
			}
					...
					struct ssdfs_btree_hierarchy *hierarchy)
{
	...
	if (search->node.id == SSDFS_BTREE_ROOT_NODE_ID) {
		...
		if (child_node_type != SSDFS_BTREE_LEAF_NODE) {
			SSDFS_ERR("invalid child node's type %#x\n",
				  child_node_type);
			return -ERANGE;
		}
	...
	if (node) {
		...
		case SSDFS_BTREE_ROOT_NODE:
			...
			if (cur_height == node_height) {
				/* do nothing */
			} else if (cur_height == (node_height + 1)) {
...
void ssdfs_debug_btree_hierarchy_object(struct ssdfs_btree_hierarchy *ptr)
{
	...
	for (i = 0; i < ptr->desc.height; i++) {
		...
		SSDFS_DBG("INSERT: op_state %#x, start_hash %llu, "
			  "end_hash %llu, "
