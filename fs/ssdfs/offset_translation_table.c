					  ...
					  int *extent_index)
{
	...
	for (i = 0; i < len; i++) {
		...
		SSDFS_DBG("ino %llu, "
			  ...
			  le64_to_cpu(pos->blk_desc.ino),
			  le32_to_cpu(pos->blk_desc.logical_offset),
			  le16_to_cpu(pos->blk_desc.peb_index),
			  le16_to_cpu(pos->blk_desc.peb_page));

		for (j = 0; j < SSDFS_BLK_STATE_OFF_MAX; j++) {
			state_off = &pos->blk_desc.state[j];
