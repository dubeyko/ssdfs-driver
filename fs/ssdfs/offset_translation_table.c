...
void ssdfs_blk2off_check_memory_leaks(void)
{
	...
	if (atomic64_read(&ssdfs_blk2off_folio_leaks) != 0) {
		SSDFS_ERR("BLK2OFF TABLE: "
			  ...
			  atomic64_read(&ssdfs_blk2off_folio_leaks));
	}
...
void ssdfs_blk2off_frag_free(void *ptr)
{
	...
	WARN_ON(atomic_read(&frag->migrating_blocks) != 0);
 ...
 * @processed_blks_bmap: bitmap of processed blocks
...
struct ssdfs_blk2off_init {
	...
	unsigned long *bmap;
	unsigned long *processed_blks_bmap;
	u32 bmap_bytes;
};
				  ...
				  u64 peb_id, size_t *buf_size)
{
	...
	ptr->peb_id = peb_id;
	atomic_set(&ptr->migrating_blocks, 0);
	atomic_set(&ptr->actual_records, 0);
			   ...
			   int state)
{
	...
	err = ssdfs_dynamic_array_create(&ptr->lblk2off,
					 SSDFS_BLK2OFF_TBL_LBLK2OFF_CAPACITY_MAX,
					 off_pos_size,
					 0xFF);
	if (unlikely(err)) {
					    ...
					    u16 logical_blk)
{
	...
	if (logical_blk >= bits_per_page) {
		new_bits_count = logical_blk + bits_per_page - 1;
		new_bits_count /= bits_per_page;
		new_bits_count *= bits_per_page;
	} else {
		new_bits_count = (logical_blk * 2) + BITS_PER_LONG - 1;
		new_bits_count /= BITS_PER_LONG;
		new_bits_count *= BITS_PER_LONG;

		new_bits_count = min_t(u32, new_bits_count, bits_per_page);
	}
...
#ifdef CONFIG_SSDFS_DEBUG
	SSDFS_DBG("new_bits_count %u, new_bytes_count %u, "
		  "logical_blk %u\n",
		  new_bits_count, new_bytes_count,
		  logical_blk);
#endif /* CONFIG_SSDFS_DEBUG */
...
#ifdef CONFIG_SSDFS_DEBUG
	SSDFS_DBG("logical_blk %u, lbmap->bits_count %u\n",
		  logical_blk, lbmap->bits_count);
#endif /* CONFIG_SSDFS_DEBUG */
...
#ifdef CONFIG_SSDFS_DEBUG
	SSDFS_DBG("logical_blk %u, found %lu\n",
		  logical_blk, found);
#endif /* CONFIG_SSDFS_DEBUG */
...
int ssdfs_blk2off_prepare_temp_bmap(struct ssdfs_blk2off_init *portion)
{
	...
	portion->processed_blks_bmap =
			ssdfs_blk2off_kvzalloc(portion->bmap_bytes,
						GFP_KERNEL);
	if (unlikely(!portion->processed_blks_bmap)) {
...
/*
 * ssdfs_blk2off_resize_temp_bmap() - resize temporary bitmap
 * @portion: initialization environment [in | out]
 */
static inline
int ssdfs_blk2off_resize_temp_bmap(struct ssdfs_blk2off_init *portion)
{
	size_t new_bmap_bytes;

#ifdef CONFIG_SSDFS_DEBUG
	BUG_ON(!portion || !portion->bmap || !portion->processed_blks_bmap);
#endif /* CONFIG_SSDFS_DEBUG */

	new_bmap_bytes = ssdfs_blk2off_table_bmap_bytes(portion->capacity);

	if (new_bmap_bytes == portion->bmap_bytes) {
#ifdef CONFIG_SSDFS_DEBUG
		SSDFS_DBG("DON'T RESIZE: bmap_bytes: old %u, new %zu\n",
			  portion->bmap_bytes, new_bmap_bytes);
#endif /* CONFIG_SSDFS_DEBUG */
		return 0;
	} else if (new_bmap_bytes < portion->bmap_bytes) {
		SSDFS_ERR("invalid bmap_bytes: old %u, new %zu\n",
			  portion->bmap_bytes, new_bmap_bytes);
		return -EINVAL;
	}

	portion->bmap = kvrealloc(portion->bmap,
				  new_bmap_bytes,
				  GFP_KERNEL | __GFP_ZERO);
	if (!portion->bmap) {
		SSDFS_ERR("fail to realloc bitmap\n");
		return -ENOMEM;
	}

	portion->processed_blks_bmap = kvrealloc(portion->processed_blks_bmap,
						 new_bmap_bytes,
						 GFP_KERNEL | __GFP_ZERO);
	if (!portion->processed_blks_bmap) {
		SSDFS_ERR("fail to realloc bitmap\n");
		return -ENOMEM;
	}
...
/*
 * ssdfs_blk2off_destroy_temp_bmap() - destroy temporary bitmap
 * @portion: initialization environment [in | out]
 */
static inline
void ssdfs_blk2off_destroy_temp_bmap(struct ssdfs_blk2off_init *portion)
{
	if (portion->bmap) {
		ssdfs_blk2off_kvfree(portion->bmap);
		portion->bmap = NULL;
	...
	if (portion->processed_blks_bmap) {
		ssdfs_blk2off_kvfree(portion->processed_blks_bmap);
		portion->processed_blks_bmap = NULL;
	}

	portion->bmap_bytes = 0;
}
...
/*
 * is_ssdfs_offset_id_reverted() - check that offset_id has been reverted
 * @offset_id1: left offset ID value
 * @offset_id2: rigth offset ID value
 *
 * The offset ID values are growing sequentially until U16_MAX value.
 * Upon achieving U16_MAX value, the offset ID value is reverted
 * to zero. This method distinguish situation when such revertion
 * takes place.
 */
static inline
bool is_ssdfs_offset_id_reverted(u16 offset_id1, u16 offset_id2)
{
	bool is_reverted = false;
	u16 diff;

	if (offset_id1 >= offset_id2)
		diff = offset_id1 - offset_id2;
	else
		diff = offset_id2 - offset_id1;

	is_reverted = diff >= SSDFS_BLK2OFF_TBL_REVERT_PIVOT;

#ifdef CONFIG_SSDFS_DEBUG
	SSDFS_DBG("offset_id1 %u, offset_id2 %u, "
		  "is_reverted %#x\n",
		  offset_id1, offset_id2,
		  is_reverted);
#endif /* CONFIG_SSDFS_DEBUG */
...
/*
 * INITIALIZED_BY_ANOTHER_THREAD() - does logical block is initialized?
 */
static inline
bool INITIALIZED_BY_ANOTHER_THREAD(struct ssdfs_blk2off_init *portion,
				   u32 logical_blk)
{
	...
	SSDFS_DBG("logical_blk %u, portion->capacity %u, "
		  "lblk2off_capacity %u\n",
		  logical_blk, portion->capacity,
		  portion->table->lblk2off_capacity);

	SSDFS_DBG("portion->processed_blks_bmap:\n");
	print_hex_dump_bytes("", DUMP_PREFIX_OFFSET,
			     portion->processed_blks_bmap,
			     portion->bmap_bytes);
	SSDFS_DBG("\n");

	SSDFS_DBG("portion->bmap:\n");
	print_hex_dump_bytes("", DUMP_PREFIX_OFFSET,
			     portion->bmap,
			     portion->bmap_bytes);
	SSDFS_DBG("\n");
	...
	found = find_next_zero_bit(portion->processed_blks_bmap,
				   portion->capacity, logical_blk);
	if (found == logical_blk) {
		found = find_next_zero_bit(portion->bmap,
					   portion->capacity,
					   logical_blk);
		if (found == logical_blk) {
			lbmap = &portion->table->lbmap;
			capacity = portion->table->lblk2off_capacity;

			is_initialized =
				!ssdfs_blk2off_table_bmap_vacant(lbmap,
...
#ifdef CONFIG_SSDFS_DEBUG
	SSDFS_DBG("logical_blk %u, is_initialized %#x\n",
		  logical_blk, is_initialized);
#endif /* CONFIG_SSDFS_DEBUG */
...
/*
 * IS_PORTION_OLD_FOR_EXTENT() - check that portion is old for extent
 */
static inline
bool IS_PORTION_OLD_FOR_EXTENT(struct ssdfs_phys_offset_table_header *pot_hdr,
				struct ssdfs_translation_extent *extent)
{
	u16 start_id;
	u16 id_count;
	...
	start_id = le16_to_cpu(pot_hdr->start_id);
	id_count = le16_to_cpu(pot_hdr->id_count);
...
/*
 * IS_EXTENT_OLD_FOR_PORTION() - check that extent is old for portion
 */
static inline
bool IS_EXTENT_OLD_FOR_PORTION(struct ssdfs_phys_offset_table_header *pot_hdr,
				struct ssdfs_translation_extent *extent)
{
	u16 start_id;
	u16 offset_id;
...
#ifdef CONFIG_SSDFS_DEBUG
	SSDFS_DBG("start_id %u, offset_id %u, len %u\n",
		  start_id, offset_id, len);
#endif /* CONFIG_SSDFS_DEBUG */

	if (is_ssdfs_offset_id_reverted(start_id, offset_id)) {
		if (start_id < SSDFS_BLK2OFF_TBL_REVERT_PIVOT &&
		    offset_id > SSDFS_BLK2OFF_TBL_REVERT_PIVOT) {
			is_old = true;
		} else {
			is_old = false;
		}
	} else {
		is_old = (offset_id + len) <= start_id;
	}
...
/*
 * ssdfs_account_processed_blocks() - account processed blocks
 */
static inline
u16 ssdfs_account_processed_blocks(struct ssdfs_blk2off_init *portion,
				    u32 logical_blk, u16 len)
{
	u32 cur_blk;
	u16 already_initialized_blks = 0;
	int i;

	for (i = 0; i < len; i++) {
		cur_blk = logical_blk + i;

		if (INITIALIZED_BY_ANOTHER_THREAD(portion, cur_blk)) {
			bitmap_set(portion->processed_blks_bmap, cur_blk, 1);
			portion->initialized_blks++;
			already_initialized_blks++;
#ifdef CONFIG_SSDFS_DEBUG
			SSDFS_DBG("portion->initialized_blks %u, "
				  "cur_block %u\n",
				  portion->initialized_blks,
				  cur_blk);
#endif /* CONFIG_SSDFS_DEBUG */
	...
	return already_initialized_blks;
}
					...
					struct ssdfs_translation_extent *extent)
{
...
#ifdef CONFIG_SSDFS_DEBUG
	SSDFS_DBG("start_id %u, id_count %u, flags %#x\n",
		  start_id, id_count, flags);
#endif /* CONFIG_SSDFS_DEBUG */
	...
	if (IS_FIRST_EXTENT_FREE(extent_index, extent)) {
	...
	if (IS_EXTENT_OLD_FOR_PORTION(&portion->pot_hdr, extent)) {
		...
		already_initialized_blks =
			ssdfs_account_processed_blocks(portion,
							logical_blk, len);

		if (already_initialized_blks > len) {
			SSDFS_ERR("already_initialized_blks %u > len %u\n",
				  already_initialized_blks, len);
			return -ERANGE;
		}

#ifdef CONFIG_SSDFS_DEBUG
		SSDFS_DBG("len %u, already_initialized_blks %u\n",
	...
	if (is_ssdfs_offset_id_reverted(start_id, offset_id)) {
		...
		ssdfs_account_processed_blocks(portion, logical_blk, id_diff);
...
#ifdef CONFIG_SSDFS_DEBUG
	SSDFS_DBG("sequence_id %u, actual_records %d\n",
		  frag->sequence_id,
		  atomic_read(&frag->actual_records));
#endif /* CONFIG_SSDFS_DEBUG */
...
/*
 * __ssdfs_process_free_translation_extent() - process free translation extent
 * @portion: pointer on portion init environment [in | out]
 * @extent_index: index of extent
 * @extent: pointer on translation extent
 *
				...
				int *extent_index)
{
	...
	used_logical_blks = le16_to_cpu(portion->pot_hdr.used_logical_blks);
	free_logical_blks = le16_to_cpu(portion->pot_hdr.free_logical_blks);
	last_allocated_blk = le16_to_cpu(portion->pot_hdr.last_allocated_blk);

	if (le16_to_cpu(portion->pot_hdr.flags) & SSDFS_INFLATED_OFF_TABLE) {
		struct ssdfs_bitmap_array *lbmap = &portion->table->lbmap;
		u32 new_capacity = used_logical_blks + free_logical_blks;

#ifdef CONFIG_SSDFS_DEBUG
		SSDFS_DBG("used_logical_blks %u, free_logical_blks %u, "
			  "last_allocated_blk %u, portion->capacity %u\n",
			  used_logical_blks, free_logical_blks,
			  last_allocated_blk, portion->capacity);
#endif /* CONFIG_SSDFS_DEBUG */

		if (new_capacity > portion->capacity) {
#ifdef CONFIG_SSDFS_DEBUG
			BUG_ON(new_capacity > U16_MAX);

			SSDFS_DBG("CHANGE CAPACITY: "
				  "new_capacity %u, portion->capacity %u\n",
				  new_capacity, portion->capacity);
#endif /* CONFIG_SSDFS_DEBUG */

			portion->capacity = (u16)new_capacity;

			err = ssdfs_blk2off_resize_temp_bmap(portion);
			if (unlikely(err)) {
				SSDFS_ERR("fail to resize temp bitmap: "
...
finish_fragment_processing:
	...
	if (portion->table->init_cno == U64_MAX) {
		...
		portion->table->used_logical_blks = used_logical_blks;
		portion->table->free_logical_blks = free_logical_blks;
		portion->table->last_allocated_blk = last_allocated_blk;

#ifdef CONFIG_SSDFS_DEBUG
		SSDFS_DBG("FIRST INIT: cno %llu, used_logical_blks %u, "
			  "free_logical_blks %u, last_allocated_blk %u\n",
			  portion->cno, used_logical_blks,
			  free_logical_blks, last_allocated_blk);
#endif /* CONFIG_SSDFS_DEBUG */
	} else if (portion->cno >= portion->table->init_cno) {
#ifdef CONFIG_SSDFS_DEBUG
		SSDFS_DBG("BEFORE: init_cno %llu, used_logical_blks %u, "
			  "free_logical_blks %u, last_allocated_blk %u\n",
			  portion->table->init_cno,
		...
		if (portion->table->last_allocated_blk > last_allocated_blk) {
			SSDFS_ERR("invalid last_allocated_blk: "
				  "value1 %u > value2 %u\n",
				  portion->table->last_allocated_blk,
				  last_allocated_blk);
			return -ERANGE;
...
#ifdef CONFIG_SSDFS_DEBUG
		SSDFS_DBG("AFTER: cno %llu, used_logical_blks %u, "
			  "free_logical_blks %u, last_allocated_blk %u\n",
			  portion->cno, used_logical_blks,
			  free_logical_blks, last_allocated_blk);
#endif /* CONFIG_SSDFS_DEBUG */
...
#ifdef CONFIG_SSDFS_DEBUG
		SSDFS_DBG("IGNORED: cno %llu, used_logical_blks %u, "
			  "free_logical_blks %u, last_allocated_blk %u\n",
			  portion->cno, used_logical_blks,
			  free_logical_blks, last_allocated_blk);
#endif /* CONFIG_SSDFS_DEBUG */
				 ...
				 u16 peb_index)
{
	...
	if (processed_blks != portion->initialized_blks) {
		SSDFS_ERR("invalid state: peb_index %u, "
			  "processed_blks %lu, initialized_blks %u, "
			  "total_blks_in_extents %u\n",
			  peb_index, processed_blks,
			  portion->initialized_blks,
			  portion->total_blks_in_extents);
		return -ERANGE;
	}
 ...
 * @table: pointer on translation table object
 * @is_clean_init: is it clean erase block initialization?
 */
static inline
					    ...
					    bool is_clean_init)
{
	...
	bmap = table->lbmap.array[SSDFS_LBMAP_INIT_INDEX];
	last_allocated_blk = table->last_allocated_blk;

	if (last_allocated_blk >= U16_MAX) {
		init_bits = bitmap_weight(bmap, capacity);
		allocated_blks = init_bits;
	} else {
	...
	if (init_bits > 0) {
		if (init_bits >= allocated_blks)
			...
			is_bitmap_initialized = false;
	} else if (is_clean_init) {
		is_bitmap_initialized = true;
	} else {
				     ...
				     u16 peb_index, u64 peb_id, u64 cno)
{
	...
	while (extent_index < portion.env->log.blk2off_tbl.extents.count) {
		...
		portion.total_blks_in_extents += le16_to_cpu(extent.len);
		++extent_index;

#ifdef CONFIG_SSDFS_DEBUG
		SSDFS_DBG("total_blks_in_extents %u, extent_index %d\n",
			  portion.total_blks_in_extents,
			  extent_index);
#endif /* CONFIG_SSDFS_DEBUG */
	};
...
#ifdef CONFIG_SSDFS_DEBUG
			SSDFS_DBG("correct capacity: "
				  "table->lblk2off_capacity %u, "
				  "portion.capacity %u\n",
...
#ifdef CONFIG_SSDFS_DEBUG
			SSDFS_DBG("logical_blk %u >= capacity %u\n",
				  logical_blk, portion.capacity);
#endif /* CONFIG_SSDFS_DEBUG */
			goto try_next_extent;
		}

		if ((logical_blk + len) >= portion.capacity) {
			struct ssdfs_bitmap_array *lbmap =
					&portion.table->lbmap;
			u32 new_capacity = logical_blk + len;

#ifdef CONFIG_SSDFS_DEBUG
			SSDFS_DBG("logical_blk %u, len %u, capacity %u\n",
				  logical_blk, len, portion.capacity);
...
try_next_extent:
	...
	if (portion.initialized_blks == 0)
		is_clean_init = true;

	err = ssdfs_define_blk2off_table_object_state(table, portion.capacity,
							is_clean_init);
	if (err) {
...
#ifdef CONFIG_SSDFS_DEBUG
	SSDFS_DBG("modification_bmap %lx\n",
...
unlock_translation_table:
	...
	ssdfs_blk2off_destroy_temp_bmap(&portion);
...
/*
 * ssdfs_blk2off_table_resize_nolock() - resize table
 * @table: pointer on translation table object
 ...
 */
static inline
int ssdfs_blk2off_table_resize_nolock(struct ssdfs_blk2off_table *table,
				      u16 new_items_count)
{
	u16 last_blk;
	int diff;
				...
				u16 new_items_count)
{
	...
	down_write(&table->translation_lock);
	err = ssdfs_blk2off_table_resize_nolock(table, new_items_count);
	up_write(&table->translation_lock);
...
int ssdfs_change_fragment_state(void *item, int old_state, int new_state)
{
...
#ifdef CONFIG_SSDFS_DEBUG
	SSDFS_DBG("sequence_id %u, state %#x, actual_records %d\n",
		  fragment->sequence_id,
		  atomic_read(&fragment->state),
		  atomic_read(&fragment->actual_records));
#endif /* CONFIG_SSDFS_DEBUG */

	state = atomic_xchg(&fragment->state, new_state);

#ifdef CONFIG_SSDFS_SAVE_WHOLE_BLK2OFF_TBL_IN_EVERY_LOG
	if (state == SSDFS_BLK2OFF_PLEASE_DELETE_FRAGMENT) {
		if (atomic_read(&fragment->actual_records) > 0) {
			/* continue logic */
		} else {
			atomic_set(&fragment->state,
	...
	case SSDFS_BLK2OFF_FRAG_DIRTY:
		...
		case SSDFS_BLK2OFF_FRAG_DIRTY:
#ifdef CONFIG_SSDFS_SAVE_WHOLE_BLK2OFF_TBL_IN_EVERY_LOG
		case SSDFS_BLK2OFF_PLEASE_DELETE_FRAGMENT:
#endif /* CONFIG_SSDFS_SAVE_WHOLE_BLK2OFF_TBL_IN_EVERY_LOG */
			/* expected old state */
			break;
				      ...
				      u16 *start_sequence_id)
{
	...
	*start_sequence_id = U16_MAX;
	...
	if (dirty_fragments == 0)
		*start_sequence_id = upper_bound;
	else if (upper_bound >= dirty_fragments)
...
/*
 * struct ssdfs_offset_id_search - offset ID search environment
 * @snapshot: pointer on table's snapshot object
 * @peb_id: PEB identification number
 * @start_sequence_id: start ID of the first available fragment
 */
struct ssdfs_offset_id_search {
	struct ssdfs_blk2off_table_snapshot *snapshot;
	u64 peb_id;
	unsigned long start_sequence_id;
};

/*
 * ssdfs_find_start_offset_id() - find start offset ID
 * @item: pointer on fragment
 * @id: fragment ID
 * @search_condition: offset ID search environment [in|out]
 *
 * This method is checking fragment with the goal
 * to find the starting offset ID
 *
 * RETURN:
 * [success]
 * [failure] - error code:
 ...
 * %-ENODATA    - stop the search.
 * %-EEXIST     - starting offset ID has been found.
 * %-EFAULT     - delete fragment.
 */
#ifdef CONFIG_SSDFS_SAVE_WHOLE_BLK2OFF_TBL_IN_EVERY_LOG
static inline
int ssdfs_find_start_offset_id(void *item, unsigned long id,
			       void *search_condition)
{
	struct ssdfs_phys_offset_table_fragment *fragment;
	struct ssdfs_offset_id_search *condition;
	u64 fragment_peb_id;
	u16 start_offset_id;
	int state;
	bool is_identical;
...
#ifdef CONFIG_SSDFS_DEBUG
	BUG_ON(!item || !search_condition);

	SSDFS_DBG("item %p, id %lu, search_condition %p\n",
		  item, id, search_condition);
#endif /* CONFIG_SSDFS_DEBUG */

	fragment = (struct ssdfs_phys_offset_table_fragment *)item;
	condition = (struct ssdfs_offset_id_search *)search_condition;

#ifdef CONFIG_SSDFS_DEBUG
	BUG_ON(!condition->snapshot);
	BUG_ON(condition->snapshot->new_sequence_id == SSDFS_INVALID_FRAG_ID);
	BUG_ON(condition->peb_id >= U64_MAX);

	SSDFS_DBG("id %lu, new_sequence_id %u, "
		  "actual_records %d\n",
		  id, condition->snapshot->new_sequence_id,
		  atomic_read(&fragment->actual_records));
#endif /* CONFIG_SSDFS_DEBUG */

	if (id >= condition->snapshot->new_sequence_id) {
#ifdef CONFIG_SSDFS_DEBUG
		SSDFS_DBG("stop search: "
			  "id %lu, new_sequence_id %u\n",
			  id,
			  condition->snapshot->new_sequence_id);
#endif /* CONFIG_SSDFS_DEBUG */
		if (condition->start_sequence_id == ULONG_MAX) {
			condition->start_sequence_id = id;
		}
		return -ENODATA;
	}
	...
	if (state == SSDFS_BLK2OFF_PLEASE_DELETE_FRAGMENT) {
		int migrating_blks = atomic_read(&fragment->migrating_blocks);
		int actual_records = atomic_read(&fragment->actual_records);

		if (migrating_blks > 0 || actual_records > 0) {
#ifdef CONFIG_SSDFS_DEBUG
			SSDFS_DBG("Don't delete fragment yet: "
				  "id %lu\n",
				  id);
#endif /* CONFIG_SSDFS_DEBUG */
			return 0;
...
#ifdef CONFIG_SSDFS_DEBUG
			SSDFS_DBG("pre-deleted fragment: "
				  "id %lu\n",
				  id);
#endif /* CONFIG_SSDFS_DEBUG */
			return -EFAULT;
	...
	if (fragment_peb_id >= U64_MAX)
		...
		is_identical = condition->peb_id == fragment_peb_id;
	start_offset_id = fragment->start_id;
	up_read(&fragment->lock);

#ifdef CONFIG_SSDFS_DEBUG
	SSDFS_DBG("id %lu, fragment_peb_id %llu, "
		  "peb_id %llu\n",
		  id, fragment_peb_id,
		  condition->peb_id);
#endif /* CONFIG_SSDFS_DEBUG */
	...
	if (is_identical) {
		err = -EEXIST;
#ifdef CONFIG_SSDFS_DEBUG
		SSDFS_DBG("found start_offset_id: "
			  "id %lu, peb_id %llu, "
			  "start_offset_id %u\n",
			  id, condition->peb_id,
			  start_offset_id);
#endif /* CONFIG_SSDFS_DEBUG */
		condition->snapshot->start_offset_id = start_offset_id;
...
/*
 * ssdfs_blk2off_table_pre_delete_fragment() - mark fragment as pre-deleted
 * @item: pointer on fragment
 * @peb_id: PEB ID
 *
 * This method tries to mark fragment as pre-deleted
 *
 * RETURN:
 * [success]
 * [failure] - error code:
 *
 * %-ERANGE     - internal logic error.
 * %-ENODATA    - stop the pre-delete operation.
 */
#ifdef CONFIG_SSDFS_SAVE_WHOLE_BLK2OFF_TBL_IN_EVERY_LOG
int ssdfs_blk2off_table_pre_delete_fragment(void *item, u64 peb_id)
{
	...
	if (fragment_peb_id >= U64_MAX) {
#ifdef CONFIG_SSDFS_DEBUG
		SSDFS_DBG("stop pre-delete: found new fragment\n");
#endif /* CONFIG_SSDFS_DEBUG */
		return -ENODATA;
	} else if (peb_id == fragment_peb_id) {
		if (atomic_read(&fragment->actual_records) > 0) {
#ifdef CONFIG_SSDFS_DEBUG
			SSDFS_DBG("DON'T pre-delete: actual_records %d\n",
				  atomic_read(&fragment->actual_records));
#endif /* CONFIG_SSDFS_DEBUG */
			return 0;
		}
		...
		down_write(&fragment->lock);
		state = atomic_xchg(&fragment->state,
				    SSDFS_BLK2OFF_PLEASE_DELETE_FRAGMENT);
		up_write(&fragment->lock);
				 ...
				 struct ssdfs_blk2off_table_snapshot *snapshot)
{
	...
	if (!ssdfs_blk2off_table_dirtied(table, peb_index)) {
		/*
		 * We need to save the offset translation table
		 * even if it is not dirty for the case of
		 * saving it for the every log.
		 */
	...
	capacity = min_t(u32, capacity, table->lblk2off_capacity);

	bmap_bytes = ssdfs_blk2off_table_bmap_bytes(table->lblk2off_capacity);
	...
	if (snapshot->new_sequence_id > 0) {
		...
		id_search.peb_id = peb_id;
		id_search.start_sequence_id = ULONG_MAX;

		err = ssdfs_sequence_array_search(sequence,
						  ssdfs_find_start_offset_id,
						  ssdfs_blk2off_frag_free,
						  &id_search);
		if (err == -ENOENT) {
			err = 0;
#ifdef CONFIG_SSDFS_DEBUG
			SSDFS_DBG("process only dirty fragments: "
				  "start_sequence_id %lu, "
				  "new_sequence_id %u\n",
				  id_search.start_sequence_id,
				  snapshot->new_sequence_id);
#endif /* CONFIG_SSDFS_DEBUG */
		} else if (unlikely(err)) {
			SSDFS_ERR("fail to find start offset ID: "
				  "err %d\n", err);
			goto finish_snapshoting;
		}

		if (id_search.start_sequence_id >= ULONG_MAX)
			start_sequence_id = snapshot->new_sequence_id;
		else
			start_sequence_id = id_search.start_sequence_id;
	} else {
...
#ifdef CONFIG_SSDFS_DEBUG
	SSDFS_DBG("start_sequence_id %d, last_sequence_id %u\n",
	...
	SSDFS_DBG("start_sequence_id %u, new_sequence_id %u, "
		  "dirty_fragments %u, last_sequence_id %u\n",
		  snapshot->start_sequence_id,
		  snapshot->new_sequence_id,
	...
	if (snapshot->dirty_fragments == 0) {
		if (ssdfs_blk2off_table_dirtied(table, peb_index)) {
			...
			/*
			 * Potentially, we have migration case here.
			 * Source PEB has offset translation table,
			 * but the first log of destination PEB
			 * requires of storing the block bitmap only.
			 */
			err = -ENODATA;
#ifdef CONFIG_SSDFS_DEBUG
			SSDFS_DBG("PEB hasn't dirty fragments\n");
#endif /* CONFIG_SSDFS_DEBUG */
			goto finish_snapshoting;
		}
	}
				    ...
				    u16 capacity, u16 *extent_count)
{
	...
	for (i = 0; i < *extent_count; i++) {
		...
		if (found->start_id == offset_id) {
			...
			if (is_ssdfs_offset_id_reverted(found->start_id,
							offset_id)) {
				/* continue search */
				err = -EAGAIN;
					...
					u16 capacity, u16 *extent_count)
{
	...
	*extent_count = 0;
	...
	do {
		...
		if (changed_area.start_lblk > sp->last_allocated_blk) {
#ifdef CONFIG_SSDFS_DEBUG
			SSDFS_DBG("ignore found area: "
				  "start_lblk %u > last_allocated_blk %u\n",
				  changed_area.start_lblk,
		...
		if ((changed_area.start_lblk + changed_area.len - 1) >
						    sp->last_allocated_blk) {
			u32 corrected_len;

			/*
			 * Correct range length
			 */
			corrected_len = (sp->last_allocated_blk + 1) -
						changed_area.start_lblk;
			changed_area.len = corrected_len;
		}

		for (i = 0; i < changed_area.len; i++) {
			bool is_extent_ended = false;

			blk = changed_area.start_lblk + i;
			pos = &sp->tbl_copy[blk];
			...
			if (is_extent_ended) {
				...
				if (unlikely(err)) {
					...
					for (j = 0; j <= i; j++) {
						blk = changed_area.start_lblk + j;
						pos = &sp->tbl_copy[blk];

						SSDFS_ERR("BLK[%d]: cno %llx, "
							  "id %u, peb_index %u, "
							  "sequence_id %u, "
							  "offset_index %u\n",
							  j,
				       ...
				       struct ssdfs_blk2off_table_snapshot *sp)
{
	...
	switch (atomic_read(&fragment->state)) {
	case SSDFS_BLK2OFF_FRAG_UNDER_COMMIT:
	case SSDFS_BLK2OFF_PLEASE_DELETE_FRAGMENT:
		/* expected state */
		break;
...
#ifdef CONFIG_SSDFS_DEBUG
		SSDFS_DBG("SSDFS_OFF_TABLE_HAS_OLD_LOG_FRAGMENT: "
			  "sequence_id %u, sp->new_sequence_id %u\n",
			  sequence_id, sp->new_sequence_id);
#endif /* CONFIG_SSDFS_DEBUG */
	...
	if (table->lblk2off_capacity > logical_blk_capacity)
		flags |= SSDFS_INFLATED_OFF_TABLE;
					...
					struct ssdfs_peb_log_offset *log_offset)
{
	...
	*compr_size = folio_size(folio) - offset_into_folio;

	kaddr = kmap_local_folio(folio, offset_into_folio);
	...
	if (err == -E2BIG) {
		*compr_size = *uncompr_size;
		SSDFS_DBG("unable to compress fragment in place: "
			  "cur_block %lu, offset_into_folio %u, "
			  "uncompr_size %zu\n",
					...
					struct ssdfs_peb_log_offset *log_offset)
{
	...
	blk2off_tbl->hdr.chain_hdr.desc_size = cpu_to_le16(desc_size);
...
int ssdfs_peb_store_blk2off_table_header(struct ssdfs_peb_info *pebi, u16 flags)
{
	...
	offset_inside_folio = reserved_offset % folio_size(folio);
				   ...
				   bool *do_not_delete_fragment)
{
	...
	*do_not_delete_fragment =
		(atomic_read(&fragment->migrating_blocks) > 0) ||
		(atomic_read(&fragment->actual_records) > 0);
	up_read(&fragment->lock);
	...
	for (; *extent_index < extent_count; ++(*extent_index)) {
		...
		if (start_offset_id == SSDFS_BLK2OFF_TABLE_INVALID_ID) {
			/*
			 * It needs to store only latest fragment.
			 */
			start_offset_id = sp->end_offset_id;
			end_offset_id = sp->end_offset_id;
			is_free_extent = true;
		} else {
 ...
 * %-ERANGE     - internal error.
 * %-ENOMEM     - fail to find memory page.
 * %-EAGAIN     - table is not completely initializied yet.
 */
				  ...
				  struct ssdfs_peb_log_offset *log_offset)
{
	...
	if (atomic_read(&table->state) <= SSDFS_BLK2OFF_OBJECT_PARTIAL_INIT) {
		SSDFS_DBG("unable to store before init finish\n");
		return -EAGAIN;
	}
...
#ifdef CONFIG_SSDFS_DEBUG
	SSDFS_DBG("fragments_count %u, start_sequence_id %u\n",
		  fragments_count,
		  snapshot.start_sequence_id);
#endif /* CONFIG_SSDFS_DEBUG */
	...
	for (i = sequence_id; i < fragments_count; i++) {
		...
		bool do_not_delete_fragment = true;
		...
		if (!need_commit) {
			...
			sequence = table->peb[peb_index].sequence;
			err = ssdfs_sequence_array_change_state(sequence,
					sequence_id,
					SSDFS_SEQUENCE_ITEM_DIRTY_TAG,
					SSDFS_SEQUENCE_MAX_TAGS,
					ssdfs_change_fragment_state,
			...
			if (err == -ENOENT) {
				...
				SSDFS_ERR("fail to set fragments as pre-deleted: "
					  "sequence_id %u, err %d\n",
					  sequence_id, err);
				goto fail_store_off_table;
			}
			...
			if (err == -ENOENT) {
				...
				SSDFS_ERR("fail to delete fragment: "
					  "sequence %d, err %d\n",
					  sequence_id, err);
				goto fail_store_off_table;
			} else
					     ...
					     struct ssdfs_offset_position *pos)
{
	...
	if (IS_ERR_OR_NULL(ptr)) {
		...
		SSDFS_ERR("fail to get fragment: "
			  "sequence_id %u, err %d\n",
			  pos->sequence_id, err);
		SSDFS_ERR("cno %llu, id %u, peb_index %u, "
			  "sequence_id %u, offset_index %u, "
			  "BLK DESC: status %#x, ino %llu, logical_offset %u, "
			    ...
			    struct ssdfs_offset_position *pos)
{
	...
	*peb_index = U16_MAX;
	...
	if (atomic_read(&table->state) <= SSDFS_BLK2OFF_OBJECT_PARTIAL_INIT) {
		SSDFS_DBG("unable to convert before initialization\n");
	...
	if (err == -EBUSY) {
		...
		do {
			...
			timeout = ktime_add_ns(ktime_get(),
						jiffies_to_nsecs(HZ));
			prepare_to_wait(&table->wait_queue,
					&wait, TASK_INTERRUPTIBLE);

			up_read(&table->translation_lock);
			schedule_hrtimeout(&timeout, HRTIMER_MODE_ABS);
			finish_wait(&table->wait_queue, &wait);
			down_read(&table->translation_lock);
...
int ssdfs_blk2off_table_assign_id(struct ssdfs_blk2off_table *table,
				  u16 logical_blk, u16 peb_index, u64 peb_id,
				  ...
				  u16 *last_sequence_id)
{
	...
	SSDFS_DBG("table %p, logical_blk %u, peb_index %u, peb_id %llu\n",
		  table, logical_blk, peb_index, peb_id);
#endif /* CONFIG_SSDFS_DEBUG */
	...
	if (fragment->peb_id >= U64_MAX) {
		/*
		 * New fragment: continue logic
		 */
	} else if (fragment->peb_id != peb_id) {
#ifdef CONFIG_SSDFS_DEBUG
		SSDFS_DBG("fragment->peb_id %llu != peb_id %llu\n",
			  fragment->peb_id, peb_id);
#endif /* CONFIG_SSDFS_DEBUG */
		return -ENOSPC;
	}
...
#ifdef CONFIG_SSDFS_DEBUG
			SSDFS_DBG("pos->sequence_id %u, last_sequence_id %u, "
				  "id %u, offset_index %u\n",
				  pos->sequence_id, *last_sequence_id,
				  pos->id, pos->offset_index);
#endif /* CONFIG_SSDFS_DEBUG */
...
unlock_fragment:
	...
	for (i = 0; i <= capacity; i++) {
		...
		if (id == SSDFS_INVALID_OFFSET_ID ||
		    pos->id == SSDFS_INVALID_OFFSET_ID) {
			/*
			 * Ignore the case
			 */
		} else if (id == pos->id) {
			...
			SSDFS_ERR("pos->sequence_id %u, last_sequence_id %u, "
				  "id %u, offset_index %u\n",
				  pos->sequence_id, *last_sequence_id,
				  id, offset_index);
	...
	if (err == -ERANGE) {
		int cur_sequence_id = *last_sequence_id;

		for (; cur_sequence_id >= 0; cur_sequence_id--) {
			...
			down_read(&fragment->lock);
			SSDFS_ERR("fragment[%d] (start_id %u, id_count %d)\n",
				  cur_sequence_id,
				  fragment->start_id,
...
/*
 * ssdfs_decrement_actual_records() - decrement fragment's actual records number
 * @table: pointer on table object
 * @peb_index: PEB's index
 * @logical_blk: logical block ID
 *
 * This method tries to decrement fragment's actual records number.
 *
 * RETURN:
 * [success]
 * [failure] - error code:
 ...
 * %-EINVAL     - invalid input
 * %-ERANGE     - internal logic error.
 * %-ENOENT     - logical block hasn't associated ID.
 */
static inline
int ssdfs_decrement_actual_records(struct ssdfs_blk2off_table *table,
				   u16 peb_index, u16 logical_blk)
{
	...
	old_pos = ssdfs_dynamic_array_get_locked(&table->lblk2off, logical_blk);
	if (IS_ERR_OR_NULL(old_pos)) {
		err = (old_pos == NULL ? -ENOENT : PTR_ERR(old_pos));
		SSDFS_ERR("fail to get logical block: "
			  "blk %u, err %d\n",
			  logical_blk, err);
		goto finish_decrement;
	}

	if (SSDFS_OFF_POS(old_pos)->id == U16_MAX) {
		err = -ENOENT;
#ifdef CONFIG_SSDFS_DEBUG
		SSDFS_DBG("logical block %d hasn't associated ID\n",
			  logical_blk);
#endif /* CONFIG_SSDFS_DEBUG */
		goto release_old_position;
	} else {
		phys_off_table = &table->peb[peb_index];
		sequence_id = SSDFS_OFF_POS(old_pos)->sequence_id;
		...
		down_write(&fragment->lock);
		actual_records = atomic_dec_return(&fragment->actual_records);
		if (actual_records == 0) {
#ifdef CONFIG_SSDFS_SAVE_WHOLE_BLK2OFF_TBL_IN_EVERY_LOG
			if (sequence_id < last_sequence_id) {
#ifdef CONFIG_SSDFS_DEBUG
				SSDFS_DBG("set fragment %lu as pre-deleted\n",
					  sequence_id);
#endif /* CONFIG_SSDFS_DEBUG */
				atomic_set(&fragment->state,
					SSDFS_BLK2OFF_PLEASE_DELETE_FRAGMENT);
...
#ifdef CONFIG_SSDFS_DEBUG
				SSDFS_DBG("DON'T set fragment %lu as pre-deleted: "
					  "last_sequence_id %lu\n",
					  sequence_id, last_sequence_id);
#endif /* CONFIG_SSDFS_DEBUG */
			}
#endif /* CONFIG_SSDFS_SAVE_WHOLE_BLK2OFF_TBL_IN_EVERY_LOG */
		} else if (actual_records < 0) {
			err = -ERANGE;
			SSDFS_WARN("invalid actual records: "
				   "sequence_id %lu, actual_records %d\n",
				   sequence_id, actual_records);
		}
		up_write(&fragment->lock);
				      ...
				      struct ssdfs_phys_offset_descriptor *off)
{
...
#ifdef CONFIG_SSDFS_TRACK_API_CALL
	SSDFS_ERR("table %p, logical_blk %u, peb_index %u, peb_id %llu, "
	...
	err = ssdfs_decrement_actual_records(table, peb_index, logical_blk);
	if (err == -ENOENT) {
		err = 0;
		/* do nothing */
	} else if (unlikely(err)) {
		SSDFS_ERR("fail to decrement actual records: "
			  "logical_blk %u, err %d\n",
			  logical_blk, err);
		goto finish_table_modification;
	}
...
#ifdef CONFIG_SSDFS_DEBUG
	SSDFS_DBG("set modification bitmap: peb_id %llu, blk %u\n",
		  peb_id, logical_blk);
	SSDFS_DBG("modification_bmap %lx\n",
		  *table->lbmap.array[SSDFS_LBMAP_MODIFICATION_INDEX]);
#endif /* CONFIG_SSDFS_DEBUG */
...
#ifdef CONFIG_SSDFS_DEBUG
	SSDFS_DBG("logical_blk %u, fragment->peb_id %llu, "
		  "POS: cno %llu, id %u, "
		  "peb_index %u, sequence_id %u, offset_index %u\n",
		  logical_blk, fragment->peb_id,
		  pos.cno, pos.id, pos.peb_index,
		  pos.sequence_id, pos.offset_index);
#endif /* CONFIG_SSDFS_DEBUG */
	...
	atomic_inc(&fragment->actual_records);
					...
					struct ssdfs_blk2off_range *extent)
{
	...
	end = min_t(unsigned long, found + len,
		    (unsigned long)lbmap->bits_count);

#ifdef CONFIG_SSDFS_DEBUG
	SSDFS_DBG("found %lu, len %u, max_blks %u, "
		  "lbmap->bits_count %u, end %lu\n",
		  found, len, max_blks,
		  lbmap->bits_count, end);
#endif /* CONFIG_SSDFS_DEBUG */
 ...
 * @table: pointer on table object
 * @len: requested length
 * @max_blk: upper bound for logical block search
					...
					struct ssdfs_blk2off_range *extent)
{
...
#ifdef CONFIG_SSDFS_TRACK_API_CALL
	SSDFS_ERR("table %p, len %u, max_blk %u, extent %p, "
		  "used_logical_blks %u, free_logical_blks %u, "
		  "last_allocated_blk %u\n",
		  table, len, max_blk, extent,
		  table->used_logical_blks,
		  table->free_logical_blks,
		  table->last_allocated_blk);
#else
	SSDFS_DBG("table %p, len %u, max_blk %u, extent %p, "
		  "used_logical_blks %u, free_logical_blks %u, "
		  "last_allocated_blk %u\n",
		  table, len, max_blk, extent,
		  table->used_logical_blks,
	...
	need_resize = table->free_logical_blks < len ||
			table->lblk2off_capacity < max_blk;

	if (need_resize) {
		u32 new_capacity;

		new_capacity = max_t(u32, max_blk,
				     table->lblk2off_capacity * 2);
		new_capacity = min_t(u32, new_capacity,
				     SSDFS_BLK2OFF_TBL_LBLK2OFF_CAPACITY_MAX);

		if (new_capacity <= table->lblk2off_capacity) {
			err = -ENODATA;
			SSDFS_DBG("bitmap hasn't vacant logical blocks: "
				  "new_capacity %u, lblk2off_capacity %u\n",
				  new_capacity,
				  table->lblk2off_capacity);
			goto finish_allocation;
		} else {
			err = ssdfs_blk2off_table_resize_nolock(table,
							(u16)new_capacity);
			if (unlikely(err)) {
				SSDFS_ERR("fail to resize table: "
					  "err %d\n", err);
				goto finish_allocation;
			}
		}
	...
	if (table->last_allocated_blk >= U16_MAX) {
		...
		start_blk = table->last_allocated_blk + 1;
		start_blk = min_t(u32, start_blk, max_blk - 1);

		if ((start_blk + len) > max_blk)
			goto try_next_range;
	}

#ifdef CONFIG_SSDFS_DEBUG
	SSDFS_DBG("last_allocated_blk %u, lblk2off_capacity %u, "
		  "max_blk %u, start_blk %u\n",
		  table->last_allocated_blk,
		  table->lblk2off_capacity,
		  max_blk, start_blk);
#endif /* CONFIG_SSDFS_DEBUG */
...
save_found_extent:
	...
	if (new_last_allocated_blk >= table->lblk2off_capacity) {
		SSDFS_ERR("extent (start_lblk %u, len %u), "
			  "new_last_allocated_blk %u, max_blk %u, "
			  "table->lblk2off_capacity %u\n",
			  extent->start_lblk, extent->len,
			  new_last_allocated_blk, max_blk,
			  table->lblk2off_capacity);
		BUG();
	}
#endif /* CONFIG_SSDFS_DEBUG */
					...
					u16 *logical_blk)
{
	...
	SSDFS_DBG("table %p, logical_blk %p, "
		  "used_logical_blks %u, free_logical_blks %u, "
		  "last_allocated_blk %u, max_blk %u\n",
		  table, logical_blk,
		  table->used_logical_blks,
		  table->free_logical_blks,
		  table->last_allocated_blk,
	...
	err = ssdfs_blk2off_table_allocate_extent(table, 1, max_blk, &extent);
	if (err) {
 ...
 * %-EPERM      - block is modified and it needs to be commited.
 */
				    ...
				    struct ssdfs_blk2off_range *extent)
{
...
#ifdef CONFIG_SSDFS_TRACK_API_CALL
	SSDFS_ERR("table %p, peb_id %llu, extent (start %u, len %u)\n",
		  table, peb_id, extent->start_lblk, extent->len);
#else
	SSDFS_DBG("table %p, peb_id %llu, extent (start %u, len %u)\n",
		  table, peb_id, extent->start_lblk, extent->len);
#endif /* CONFIG_SSDFS_TRACK_API_CALL */
	...
	if (!is_vacant) {
		err = -EPERM;
#ifdef CONFIG_SSDFS_DEBUG
		SSDFS_DBG("unable to free without commit: "
			  "extent (start %u, len %u)\n",
			  extent->start_lblk, extent->len);
#endif /* CONFIG_SSDFS_DEBUG */
		goto finish_freeing;
	...
	for (i = extent->start_lblk; i < end_lblk; i++) {
		err = ssdfs_decrement_actual_records(table, peb_index, i);
		if (err == -ENOENT) {
			SSDFS_WARN("logical block %d hasn't associated ID\n",
				  i);
		} else if (unlikely(err)) {
			SSDFS_ERR("fail to decrement fragment's actual records: "
				  "err %d\n", err);
			goto finish_freeing;
		}
		...
		if (err == -ENOSPC) {
			...
			if (unlikely(err)) {
				SSDFS_ERR("fail to add fragment: "
					  "peb_index %u, peb_id %llu, err %d\n",
					  peb_index, peb_id, err);
				goto finish_freeing;
			}
			...
			if (unlikely(err)) {
				SSDFS_ERR("fail to assign id: "
					  "peb_index %u, peb_id %llu, "
					  "logical_blk %u, err %d\n",
					  peb_index, peb_id, i, err);
				goto finish_freeing;
			}
		} else if (err == -ENOENT) {
#ifdef CONFIG_SSDFS_DEBUG
...
finish_fragment_modification:
		...
		if (unlikely(err)) {
			SSDFS_ERR("fail to copy: err %d\n",
				  err);
			ssdfs_dynamic_array_release(&table->lblk2off,
						    i, kaddr);
			goto finish_freeing;
		}
				   ...
				   u16 logical_blk)
{
	...
	err = ssdfs_blk2off_table_free_extent(table, peb_index,
						peb_id, &extent);
	if (err == -EAGAIN) {
#ifdef CONFIG_SSDFS_DEBUG
		SSDFS_DBG("unable to free before initialization: "
...
/*
 * is_ssdfs_blk2off_table_extent_modified() - is extent modified
 * @table: pointer on table object
 * @peb_index: PEB's index
 * @extent: pointer on extent
 */
...
/*
 * ssdfs_save_logical_block_content() - copy logical block's content
 * @fsi: pointer on shared file system object
 * @block_index: block index in request's extent
 * @migrating_blk: pointer on migrating block [out]
 * @req: request's result with block's content [in|out]
 *
 * This method tries to save the logical block's content.
 *
 * RETURN:
 * [success]
 * [failure] - error code:
 ...
 */
static
int ssdfs_save_logical_block_content(struct ssdfs_fs_info *fsi,
				     u16 block_index,
				     struct ssdfs_migrating_block *migrating_blk,
				     struct ssdfs_segment_request *req)
{
...
#ifdef CONFIG_SSDFS_DEBUG
	BUG_ON(!fsi || !migrating_blk || !req);

	SSDFS_DBG("fsi %p, block_index %u, "
		  "migrating_blk %p, req %p\n",
		  fsi, block_index, migrating_blk, req);
#endif /* CONFIG_SSDFS_DEBUG */
	...
	while (allocation_size > 0) {
		...
		if (IS_ERR_OR_NULL(folio)) {
			...
			ssdfs_blk2off_folio_batch_release(&migrating_blk->batch);
			return err;
		}

#ifdef CONFIG_SSDFS_DEBUG
		SSDFS_DBG("folio %p, count %d, folio_size %zu\n",
			  folio,
		...
		while (processed_bytes < folio_size(folio)) {
			kaddr = kmap_local_folio(folio, processed_bytes);
			err = ssdfs_unaligned_read_folio_batch(&state->batch,
								offset,
								PAGE_SIZE,
								kaddr);
			kunmap_local(kaddr);
		...
		folio_batch_add(&migrating_blk->batch, folio);

		allocation_size -= folio_size(folio);
	}
					    ...
					    struct ssdfs_segment_request *req)
{
...
#ifdef CONFIG_SSDFS_DEBUG
	BUG_ON(logical_blk_diff >= count);
#endif /* CONFIG_SSDFS_DEBUG */

	err = ssdfs_save_logical_block_content(fsi, logical_blk_diff, blk, req);
	if (unlikely(err)) {
		SSDFS_ERR("fail to save logical block's content: "
			  "block_index %u, err %d\n",
			  logical_blk_diff, err);
		goto finish_set_block_migration;
	}

	blk->state = SSDFS_LBLOCK_UNDER_MIGRATION;
	blk->peb_index = peb_index;
	blk->sequence_id = U16_MAX;
	...
	blk->sequence_id = pos.sequence_id;
	phys_off_table = &table->peb[pos.peb_index];
	...
	down_read(&fragment->lock);
	atomic_inc(&fragment->migrating_blocks);
	up_read(&fragment->lock);

#ifdef CONFIG_SSDFS_DEBUG
	SSDFS_DBG("migrating blocks %d\n",
		  atomic_read(&fragment->migrating_blocks));
#endif /* CONFIG_SSDFS_DEBUG */
...
/*
 * ssdfs_retrieve_logical_block_content() - retrieve logical block's content
 * @fsi: pointer on shared file system object
 * @block_index: block index in request's extent
 * @migrating_blk: pointer on migrating block [in]
 * @req: request's result with block's content [in|out]
 *
 * This method tries to retrieve the logical block's content.
 *
 * RETURN:
 * [success]
 * [failure] - error code:
 ...
 */
static
int ssdfs_retrieve_logical_block_content(struct ssdfs_fs_info *fsi,
				     u16 block_index,
				     struct ssdfs_migrating_block *migrating_blk,
				     struct ssdfs_segment_request *req)
{
	...
	if (folio_batch_count(&migrating_blk->batch) == 0) {
		SSDFS_ERR("empty migrating block\n");
		return -ERANGE;
	}

	offset = 0;
	read_size = fsi->pagesize;

	while (read_size > 0) {
		void *kaddr;

		for (i = 0; i < folio_batch_count(&migrating_blk->batch); i++) {
			u32 processed_bytes = 0;

			folio = migrating_blk->batch.folios[i];
			...
			SSDFS_DBG("folio %p, count %d, folio_size %zu\n",
				  ...
				  folio_ref_count(folio),
				  folio_size(folio));
			SSDFS_DBG("offset %u, read_size %u, "
				  "processed_bytes %u, folio_size %zu\n",
				  offset, read_size,
				  processed_bytes,
				  folio_size(folio));
#endif /* CONFIG_SSDFS_DEBUG */
			...
			while (processed_bytes < folio_size(folio)) {
				...
				if (unlikely(err)) {
					SSDFS_ERR("fail to write folio batch: "
						  "offset %u, processed_bytes %u, "
						  "folio_size %zu, read_size %u, "
						  "index %d, err %d\n",
						  offset, processed_bytes,
						  folio_size(folio), read_size,
						  i, err);
					return err;
				}
					...
					struct ssdfs_segment_request *req)
{
	...
	err = ssdfs_retrieve_logical_block_content(fsi, processed_blks,
						   blk, req);
	if (unlikely(err)) {
		SSDFS_ERR("fail to retrieve logical block's content: "
			  "logical_blk %u, err %d\n",
			  logical_blk, err);
		goto finish_get_block_state;
	}
...
/*
 * ssdfs_update_migrating_block_content() - update logical block's content
 * @fsi: pointer on shared file system object
 * @block_index: block index in request's extent
 * @migrating_blk: pointer on migrating block [out]
 * @req: request's result with block's content [in|out]
 *
 * This method tries to update migrating state of logical block's content.
 *
 * RETURN:
 * [success]
 * [failure] - error code:
 ...
 */
static
int ssdfs_update_migrating_block_content(struct ssdfs_fs_info *fsi,
				     u16 block_index,
				     struct ssdfs_migrating_block *migrating_blk,
				     struct ssdfs_segment_request *req)
{
					   ...
					   struct ssdfs_segment_request *req)
{
	...
	err = ssdfs_update_migrating_block_content(fsi, processed_blks,
						   blk, req);
	if (unlikely(err)) {
		SSDFS_ERR("fail to update migrating block's content: "
			  "logical_blk %u, err %d\n",
			  logical_blk, err);
		goto finish_update_block_state;
	}
						...
						u16 peb_index)
{
	...
	for (i = 0; i <= tbl->last_allocated_blk; i++) {
		...
		if (blk->state == SSDFS_LBLOCK_UNDER_COMMIT) {
			...
			sequence_id = blk->sequence_id;

			if (sequence_id >= U16_MAX) {
				SSDFS_WARN("invalid sequence_id: "
					   "logical_blk %u\n",
					   i);
			}
			...
			phys_off_table = &tbl->peb[peb_index];
			...
			if (atomic_dec_return(&fragment->migrating_blocks) < 0) {
				err = -ERANGE;
				SSDFS_WARN("invalid migrating_blocks: "
					   "logical_blk %d, sequence_id %u, "
					   "migrating_blocks %d\n",
					   i, sequence_id,
					   atomic_read(&fragment->migrating_blocks));
			}
