...
atomic64_t ssdfs_seg_bmap_folio_leaks;
 ...
 * struct folio *ssdfs_seg_bmap_alloc_folio(gfp_t gfp_mask,
 *                                          unsigned int order)
 * struct folio *ssdfs_seg_bmap_add_batch_folio(struct folio_batch *batch,
 *                                              unsigned int order)
 * void ssdfs_seg_bmap_free_folio(struct folio *folio)
 * void ssdfs_seg_bmap_folio_batch_release(struct folio_batch *batch)
 */
#ifdef CONFIG_SSDFS_MEMORY_LEAKS_ACCOUNTING
...
{
#ifdef CONFIG_SSDFS_MEMORY_LEAKS_ACCOUNTING
	atomic64_set(&ssdfs_seg_bmap_folio_leaks, 0);
...
{
#ifdef CONFIG_SSDFS_MEMORY_LEAKS_ACCOUNTING
	if (atomic64_read(&ssdfs_seg_bmap_folio_leaks) != 0) {
		SSDFS_ERR("SEGMENT BITMAP: "
			  "memory leaks include %lld folios\n",
			  atomic64_read(&ssdfs_seg_bmap_folio_leaks));
	}
					...
					struct folio *folio)
{
	...
	SSDFS_DBG("seg %llu, peb_index %u, folio %p\n",
				...
				int state)
{
...
#ifdef CONFIG_SSDFS_DEBUG
	BUG_ON(!pebc || !pebc->parent_si || !pebc->parent_si->fsi);
	BUG_ON(!pebc->parent_si->fsi->segbmap || !folio);
	...
	SSDFS_ERR("seg %llu, peb_index %u, "
		  "sequence_id %u, folio %p, "
		  "state %#x\n",
		  pebc->parent_si->seg_id, pebc->peb_index,
		  sequence_id, folio, state);
#else
	...
	if (desc->state != SSDFS_SEGBMAP_FRAG_CREATED) {
		...
		hdr = SSDFS_SBMP_FRAG_HDR(kmap_local_folio(folio, 0));
 ...
 * @folio_index: index of folio in request
 * @req: segment request
 *
				      ...
				      struct ssdfs_segment_request *req)
{
	...
	if (IS_ERR(sfolio)) {
		SSDFS_ERR("fail to find folio: "
			  ...
			  fragment_index, PTR_ERR(sfolio));
		return PTR_ERR(sfolio);
	} else if (!sfolio) {
		SSDFS_ERR("fail to find folio: fragment_index %u\n",
			  fragment_index);
		return -ERANGE;
	}
	...
	__ssdfs_memcpy_to_folio(dfolio, 0, PAGE_SIZE,
			        kaddr, 0, PAGE_SIZE,
			        PAGE_SIZE);
...
fail_copy_fragment:
	...
	ssdfs_folio_put(sfolio);

#ifdef CONFIG_SSDFS_DEBUG
	SSDFS_DBG("folio %p, count %d\n",
		  sfolio, folio_ref_count(sfolio));
#endif /* CONFIG_SSDFS_DEBUG */
				     ...
				     struct ssdfs_segment_request *req2)
{
	...
	kaddr = kmap_local_folio(dfolio, 0);
	hdr = SSDFS_SBMP_FRAG_HDR(kaddr);
					 ...
					 unsigned long dirty_bmap)
{
	...
	do {
		...
		if (has_backup) {
			ssdfs_request_init(req2, segbmap->fsi->pagesize);
			    ...
			    u64 seg, struct completion **end)
{
	...
	*end = &segbmap->desc_array[fragment_index].init_end;
	...
	if (IS_ERR(folio)) {
		...
		err = -ERANGE;
		SSDFS_ERR("fail to get fragment %lu folio\n",
			  fragment_index);
		goto finish_get_state;
	}
	...
	if (seg < folio_item) {
		err = -ERANGE;
		SSDFS_ERR("seg %llu < folio_item %llu\n",
			  seg, folio_item);
		goto free_folio;
	}

	folio_item = seg - folio_item;

	if (folio_item >= ssdfs_segbmap_items_per_fragment(fragment_size)) {
		err = -ERANGE;
		SSDFS_ERR("invalid folio_item %llu\n",
			  folio_item);
		goto free_folio;
	}

	byte_offset = ssdfs_segbmap_get_item_byte_offset(folio_item);
	...
	byte_item = folio_item - ((byte_offset - hdr_size) * items_per_byte);
				 ...
				 u16 fragment_size)
{
	...
	div_u64_rem(folio_item, items_per_byte, &byte_item);
