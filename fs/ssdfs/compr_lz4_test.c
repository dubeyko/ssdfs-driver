 ...
 *
 * fs/ssdfs/compr_lz4_test.c - KUnit tests for LZ4 compression.
 *
...
/*
 * Test LZ4 initialization and cleanup
 */
static void test_lz4_init_exit(struct kunit *test)
{
	int err;
	...
	/* Verify LZ4 compressor is registered */
	KUNIT_EXPECT_NOT_NULL(test, ssdfs_compressors[SSDFS_COMPR_LZ4]);
	if (ssdfs_compressors[SSDFS_COMPR_LZ4]) {
		KUNIT_EXPECT_STREQ(test, ssdfs_compressors[SSDFS_COMPR_LZ4]->name, "lz4");
		KUNIT_EXPECT_EQ(test, ssdfs_compressors[SSDFS_COMPR_LZ4]->type, SSDFS_COMPR_LZ4);
		KUNIT_EXPECT_NOT_NULL(test, ssdfs_compressors[SSDFS_COMPR_LZ4]->compr_ops);
	}

	/* Test LZ4 cleanup */
	ssdfs_lz4_exit();

	/* Verify LZ4 compressor is unregistered */
	KUNIT_EXPECT_NULL(test, ssdfs_compressors[SSDFS_COMPR_LZ4]);
}

/*
 * Test LZ4 workspace allocation and deallocation
 */
static void test_lz4_workspace_alloc_free(struct kunit *test)
{
	...
	ops = ssdfs_compressors[SSDFS_COMPR_LZ4]->compr_ops;
...
/*
 * Test LZ4 compression with different data patterns
 */
static void test_lz4_compress_zeros(struct kunit *test)
{
	...
	err = ssdfs_compress(SSDFS_COMPR_LZ4, data_in, cdata_out,
			     &srclen, &destlen);
	KUNIT_EXPECT_EQ(test, err, 0);
...
static void test_lz4_compress_ones(struct kunit *test)
{
	unsigned char *data_in, *cdata_out;
	size_t srclen = PAGE_SIZE;
	size_t destlen = PAGE_SIZE;
...
static void test_lz4_compress_repetitive(struct kunit *test)
{
	unsigned char *data_in, *cdata_out;
	size_t srclen = PAGE_SIZE;
	size_t destlen = PAGE_SIZE;
...
static void test_lz4_compress_random(struct kunit *test)
{
	unsigned char *data_in, *cdata_out;
	size_t srclen = PAGE_SIZE;
	size_t destlen = PAGE_SIZE;
...
static void test_lz4_compress_small_buffer(struct kunit *test)
{
	unsigned char *data_in, *cdata_out;
	size_t srclen = PAGE_SIZE;
 ...
 */
static void test_lz4_decompress_valid_data(struct kunit *test)
{
	...
	/* Now decompress */
	err = ssdfs_decompress(SSDFS_COMPR_LZ4, cdata_out, data_decompressed,
...
static void test_lz4_decompress_alternating_pattern(struct kunit *test)
{
	unsigned char *data_in, *cdata_out, *data_decompressed;
	size_t srclen = PAGE_SIZE;
	size_t destlen = PAGE_SIZE;
...
static void test_lz4_decompress_corrupted_data(struct kunit *test)
{
	...
	err = ssdfs_decompress(SSDFS_COMPR_LZ4, cdata_in, data_out,
			       srclen, destlen);
...
static void test_lz4_decompress_wrong_size(struct kunit *test)
{
	unsigned char *data_in, *cdata_out, *data_decompressed;
	size_t srclen = PAGE_SIZE;
	size_t destlen = PAGE_SIZE;
 ...
 */
static void test_lz4_multiple_cycles(struct kunit *test)
{
	...
	for (i = 0; i < 6; i++) {
		...
		/* Compress */
		err = ssdfs_compress(SSDFS_COMPR_LZ4, data_in, cdata_tmp,
				     &srclen, &destlen);
		if (err == -E2BIG)
		...
		/* Decompress */
		err = ssdfs_decompress(SSDFS_COMPR_LZ4, cdata_tmp, data_out,
				       compressed_size, PAGE_SIZE);
		KUNIT_EXPECT_EQ(test, err, 0);
 ...
 */
static void test_lz4_different_sizes(struct kunit *test)
{
	unsigned char *data_in, *cdata_out, *data_decompressed;
 ...
 */
static struct kunit_case ssdfs_compr_lz4_test_cases[] = {
	KUNIT_CASE(test_lz4_init_exit),
	KUNIT_CASE(test_lz4_workspace_alloc_free),
	KUNIT_CASE(test_lz4_compress_zeros),
	KUNIT_CASE(test_lz4_compress_ones),
	KUNIT_CASE(test_lz4_compress_repetitive),
	KUNIT_CASE(test_lz4_compress_random),
	KUNIT_CASE(test_lz4_compress_small_buffer),
	KUNIT_CASE(test_lz4_decompress_valid_data),
	KUNIT_CASE(test_lz4_decompress_alternating_pattern),
	KUNIT_CASE(test_lz4_decompress_corrupted_data),
	KUNIT_CASE(test_lz4_decompress_wrong_size),
	KUNIT_CASE(test_lz4_multiple_cycles),
	KUNIT_CASE(test_lz4_different_sizes),
	{}
};

static int ssdfs_compr_lz4_test_init(struct kunit *test)
{
	/* Initialize compression subsystem */
	return ssdfs_compressors_init();
}

static void ssdfs_compr_lz4_test_exit(struct kunit *test)
{
	/* Cleanup compression subsystem */
	ssdfs_compressors_exit();
}

static struct kunit_suite ssdfs_compr_lz4_test_suite = {
	.name = "ssdfs-compr-lz4",
	.init = ssdfs_compr_lz4_test_init,
	.exit = ssdfs_compr_lz4_test_exit,
	.test_cases = ssdfs_compr_lz4_test_cases,
};

kunit_test_suite(ssdfs_compr_lz4_test_suite);

MODULE_LICENSE("Dual BSD/GPL");
MODULE_AUTHOR("Viacheslav Dubeyko <slava@dubeyko.com>");
MODULE_DESCRIPTION("KUnit tests for SSDFS LZ4 compression");
