...
int ssdfs_xattrs_btree_desc_flush(struct ssdfs_btree *tree)
{
	...
	memcpy(&fsi->vh->xattr_btree.desc, &desc,
...
/*
 * ssdfs_xattrs_btree_node_move_items_range() - move range between nodes
 * @src: source node
 ...
 */
static
int ssdfs_xattrs_btree_node_move_items_range(struct ssdfs_btree_node *src,
					    ...
					    u16 start_item, u16 count)
{
	...
	err = ssdfs_xattrs_btree_node_extract_range(src,
						    start_item, count,
						    search);
	if (unlikely(err)) {
		SSDFS_ERR("fail to extract range: "
	...
	err = ssdfs_xattrs_btree_node_delete_range(src, search);
	if (unlikely(err)) {
		SSDFS_ERR("fail to delete range: "
			  "node_id %u, start_item %u, "
			  "count %u, err %d\n",
	...
	err = ssdfs_xattrs_btree_node_find_range(dst, search);
	if (unlikely(err)) {
	...
	err = ssdfs_xattrs_btree_node_insert_range(dst, search);
	if (unlikely(err)) {
		SSDFS_ERR("fail to insert range: "
			  "node_id %u, err %d\n",
			  dst->node_id, err);
...
const struct ssdfs_btree_node_operations ssdfs_xattrs_btree_node_ops = {
	...
	.move_items_range	= ssdfs_xattrs_btree_node_move_items_range,
