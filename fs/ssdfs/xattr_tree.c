...
/*
 * void ssdfs_xattr_cache_leaks_increment(void *kaddr)
 * void ssdfs_xattr_cache_leaks_decrement(void *kaddr)
 * void *ssdfs_xattr_kmalloc(size_t size, gfp_t flags)
...
#else
	SSDFS_MEMORY_ALLOCATOR_FNS(xattr)
#endif /* CONFIG_SSDFS_MEMORY_LEAKS_ACCOUNTING */
...
void ssdfs_xattr_check_memory_leaks(void)
{
	...
	if (atomic64_read(&ssdfs_xattr_folio_leaks) != 0) {
		SSDFS_ERR("XATTR TREE: "
			  ...
			  atomic64_read(&ssdfs_xattr_folio_leaks));
	}

	if (atomic64_read(&ssdfs_xattr_memory_leaks) != 0) {
		SSDFS_ERR("XATTR TREE: "
			  "memory allocator suffers from %lld leaks\n",
			  atomic64_read(&ssdfs_xattr_memory_leaks));
	}

	if (atomic64_read(&ssdfs_xattr_cache_leaks) != 0) {
		SSDFS_ERR("XATTR TREE: "
			  "caches suffers from %lld leaks\n",
			  atomic64_read(&ssdfs_xattr_cache_leaks));
	}
#endif /* CONFIG_SSDFS_MEMORY_LEAKS_ACCOUNTING */
}
			     ...
			     struct ssdfs_inode_info *ii)
{
	...
	ptr = ssdfs_xattr_kzalloc(sizeof(struct ssdfs_xattrs_btree_info),
				  GFP_KERNEL);
	if (!ptr) {
	...
	if (inline_capacity == 0 || inline_capacity >= U16_MAX) {
	...
	if (ptr->inline_capacity == 1) {
		...
		ptr->inline_xattrs =
			ssdfs_xattr_kzalloc(xattr_size * ptr->inline_capacity,
					    GFP_KERNEL);
...
void ssdfs_xattrs_tree_destroy(struct ssdfs_inode_info *ii)
{
	...
	ssdfs_xattr_kfree(ii->xattrs_tree);
	ii->xattrs_tree = NULL;
...
/*
 * ssdfs_generate_xattr_name_hash() - generate a name's hash
...
u64 ssdfs_generate_xattr_name_hash(const char *name, size_t len)
{
	...
	return __ssdfs_generate_name_hash(name, len,
...
#ifdef CONFIG_SSDFS_DEBUG
	SSDFS_DBG("request (flags %#x, start_hash %llx, "
		  "name %s, name_len %zu), "
		  "found (flags %#x, hash %llx, name_len %u)\n",
		  req_flags, start_hash, req_name, req_name_len,
		  flags, hash_code, name_len);
#endif /* CONFIG_SSDFS_DEBUG */
...
finish_check_xattr:
#ifdef CONFIG_SSDFS_DEBUG
	SSDFS_DBG("result.state %#x, err %d\n",
			   ...
			   struct ssdfs_btree_search *search)
{
	...
	name_hash = ssdfs_generate_xattr_name_hash(name, len);
 ...
 */
static
int ssdfs_define_name_type(int name_index,
			   ...
			   u8 *name_type)
{
	...
	SSDFS_DBG("name_len %zu, name_index %#x\n",
		  name_len, name_index);
#endif /* CONFIG_SSDFS_DEBUG */
	...
	if (can_name_be_inline(name_len)) {
		...
		if (res != 0) {
			switch (name_index) {
			case SSDFS_USER_XATTR_ID:
				*name_type = SSDFS_XATTR_USER_INLINE_NAME;
				return 0;

			case SSDFS_TRUSTED_XATTR_ID:
				*name_type = SSDFS_XATTR_TRUSTED_INLINE_NAME;
				return 0;

			case SSDFS_SECURITY_XATTR_ID:
				*name_type = SSDFS_XATTR_SECURITY_INLINE_NAME;
				return 0;

			case SSDFS_SYSTEM_XATTR_ID:
				*name_type = SSDFS_XATTR_SYSTEM_INLINE_NAME;
				return 0;

			case SSDFS_POSIX_ACL_ACCESS_XATTR_ID:
			case SSDFS_POSIX_ACL_DEFAULT_XATTR_ID:
			case SSDFS_RICHACL_XATTR_ID:
				/* TODO: support */
				break;
			}
			     ...
			     struct ssdfs_blob_extent *desc)
{
	...
	blks_count = (size + fsi->pagesize - 1) / fsi->pagesize;
	for (i = 0; i < blks_count; i++) {
		u32 processed_bytes = 0;

		while (processed_bytes < fsi->pagesize) {
			folio = ssdfs_request_allocate_and_add_folio(i, req);
			...
			cur_len = min_t(size_t, folio_size(folio),
					size - copied_data);
...
finish_save_external_blob:
	for (i = 0; i < req->result.content.count; i++) {
		...
		ssdfs_xattr_folio_batch_release(&blk_state->batch);
	}
			...
			struct ssdfs_btree_search *search)
{
	...
	name_hash = ssdfs_generate_xattr_name_hash(name, name_len);
	if (name_hash == U64_MAX) {
		SSDFS_ERR("fail to generate name hash\n");
		return -ERANGE;
	}
	...
	err = ssdfs_define_name_type(name_index, name, name_len,
				     &name_type);
	if (unlikely(err)) {
	...
	xattr->header.name_len = (u8)inline_len;
				...
				struct ssdfs_btree *tree)
{
	...
	if (le16_to_cpu(desc->index_area_min_size) < (4 * xattr_size)) {
		err = -EIO;
...
int ssdfs_xattrs_btree_create_node(struct ssdfs_btree_node *node)
{
	...
	case SSDFS_BTREE_INDEX_NODE:
		...
		node->raw.xattrs_header.free_space = cpu_to_le16(0);
		break;
	...
	SSDFS_DBG("node_id %u, xattrs_count %u, free_space %u\n",
		  ...
		  le16_to_cpu(node->raw.xattrs_header.free_space));
					...
					struct ssdfs_xattr_entry *xattr)
{
	...
	err = ssdfs_memcpy_from_batch(xattr, 0, item_size,
				      batch, src_offset,
				      item_size);
	if (unlikely(err)) {
...
#ifdef CONFIG_SSDFS_DEBUG
	SSDFS_DBG("search->result.start_index %u, direction %#x\n",
		  search->result.start_index,
		  direction);
#endif /* CONFIG_SSDFS_DEBUG */
			     ...
			     u16 start_index)
{
	...
	lookup_index = ssdfs_convert_item2lookup_index(node->node_size,
						       start_index);
	if (unlikely(lookup_index >= SSDFS_XATTRS_BTREE_LOOKUP_TABLE_SIZE)) {
	...
	if (item_index > 0) {
		...
		if (cur_hash < start_hash) {
			...
			for (i = 0; i < items_area.items_count; i++) {
				err1 = ssdfs_xattrs_btree_node_get_xattr(node,
								   &items_area,
								   i, &xattr);
				if (unlikely(err1)) {
				...
				SSDFS_ERR("index %d, name_hash %llx\n",
					  i,
					 ...
					 u32 new_size)
{
	...
	xattrs_header = &node->raw.xattrs_header;
	xattrs_header->free_space =
		cpu_to_le16((u16)node->items_area.free_space);
