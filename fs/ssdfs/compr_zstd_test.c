 ...
 *
 * fs/ssdfs/compr_zstd_test.c - KUnit tests for ZSTD compression.
 *
...
/*
 * Test ZSTD initialization and cleanup
 */
static void test_zstd_init_exit(struct kunit *test)
{
	int err;

	/* Test ZSTD initialization */
	err = ssdfs_zstd_init();
	KUNIT_EXPECT_EQ(test, err, 0);

	/* Verify ZSTD compressor is registered */
	KUNIT_EXPECT_NOT_NULL(test, ssdfs_compressors[SSDFS_COMPR_ZSTD]);
	if (ssdfs_compressors[SSDFS_COMPR_ZSTD]) {
		KUNIT_EXPECT_STREQ(test, ssdfs_compressors[SSDFS_COMPR_ZSTD]->name, "zstd");
		KUNIT_EXPECT_EQ(test, ssdfs_compressors[SSDFS_COMPR_ZSTD]->type, SSDFS_COMPR_ZSTD);
		KUNIT_EXPECT_NOT_NULL(test, ssdfs_compressors[SSDFS_COMPR_ZSTD]->compr_ops);
	}

	/* Test ZSTD cleanup */
	ssdfs_zstd_exit();

	/* Verify ZSTD compressor is unregistered */
	KUNIT_EXPECT_NULL(test, ssdfs_compressors[SSDFS_COMPR_ZSTD]);
}

/*
 * Test ZSTD workspace allocation and deallocation
 */
static void test_zstd_workspace_alloc_free(struct kunit *test)
{
	...
	ops = ssdfs_compressors[SSDFS_COMPR_ZSTD]->compr_ops;
...
/*
 * Test ZSTD compression with different data patterns
 */
static void test_zstd_compress_zeros(struct kunit *test)
{
	...
	err = ssdfs_compress(SSDFS_COMPR_ZSTD, data_in, cdata_out,
			     &srclen, &destlen);
	KUNIT_EXPECT_EQ(test, err, 0);
...
static void test_zstd_compress_ones(struct kunit *test)
{
	unsigned char *data_in, *cdata_out;
	size_t srclen = PAGE_SIZE;
	size_t destlen = PAGE_SIZE;
...
static void test_zstd_compress_repetitive(struct kunit *test)
{
	unsigned char *data_in, *cdata_out;
	size_t srclen = PAGE_SIZE;
	size_t destlen = PAGE_SIZE;
...
static void test_zstd_compress_random(struct kunit *test)
{
	unsigned char *data_in, *cdata_out;
	size_t srclen = PAGE_SIZE;
	size_t destlen = PAGE_SIZE;
...
static void test_zstd_compress_small_buffer(struct kunit *test)
{
	unsigned char *data_in, *cdata_out;
	size_t srclen = PAGE_SIZE;
 ...
 */
static void test_zstd_decompress_valid_data(struct kunit *test)
{
	...
	/* Now decompress */
	err = ssdfs_decompress(SSDFS_COMPR_ZSTD, cdata_out, data_decompressed,
...
static void test_zstd_decompress_alternating_pattern(struct kunit *test)
{
	unsigned char *data_in, *cdata_out, *data_decompressed;
	size_t srclen = PAGE_SIZE;
	size_t destlen = PAGE_SIZE;
...
static void test_zstd_decompress_corrupted_data(struct kunit *test)
{
	...
	err = ssdfs_decompress(SSDFS_COMPR_ZSTD, cdata_in, data_out,
			       srclen, destlen);
...
static void test_zstd_decompress_wrong_size(struct kunit *test)
{
	unsigned char *data_in, *cdata_out, *data_decompressed;
	size_t srclen = PAGE_SIZE;
	size_t destlen = PAGE_SIZE;
 ...
 */
static void test_zstd_multiple_cycles(struct kunit *test)
{
	...
	for (i = 0; i < 6; i++) {
		...
		/* Compress */
		err = ssdfs_compress(SSDFS_COMPR_ZSTD, data_in, cdata_tmp,
				     &srclen, &destlen);
		if (err == -E2BIG)
		...
		/* Decompress */
		err = ssdfs_decompress(SSDFS_COMPR_ZSTD, cdata_tmp, data_out,
				       compressed_size, PAGE_SIZE);
		KUNIT_EXPECT_EQ(test, err, 0);
 ...
 */
static void test_zstd_different_sizes(struct kunit *test)
{
	unsigned char *data_in, *cdata_out, *data_decompressed;
 ...
 */
static struct kunit_case ssdfs_compr_zstd_test_cases[] = {
	KUNIT_CASE(test_zstd_init_exit),
	KUNIT_CASE(test_zstd_workspace_alloc_free),
	KUNIT_CASE(test_zstd_compress_zeros),
	KUNIT_CASE(test_zstd_compress_ones),
	KUNIT_CASE(test_zstd_compress_repetitive),
	KUNIT_CASE(test_zstd_compress_random),
	KUNIT_CASE(test_zstd_compress_small_buffer),
	KUNIT_CASE(test_zstd_decompress_valid_data),
	KUNIT_CASE(test_zstd_decompress_alternating_pattern),
	KUNIT_CASE(test_zstd_decompress_corrupted_data),
	KUNIT_CASE(test_zstd_decompress_wrong_size),
	KUNIT_CASE(test_zstd_multiple_cycles),
	KUNIT_CASE(test_zstd_different_sizes),
	{}
};

static int ssdfs_compr_zstd_test_init(struct kunit *test)
{
	/* Initialize compression subsystem */
	return ssdfs_compressors_init();
}

static void ssdfs_compr_zstd_test_exit(struct kunit *test)
{
	/* Cleanup compression subsystem */
	ssdfs_compressors_exit();
}

static struct kunit_suite ssdfs_compr_zstd_test_suite = {
	.name = "ssdfs-compr-zstd",
	.init = ssdfs_compr_zstd_test_init,
	.exit = ssdfs_compr_zstd_test_exit,
	.test_cases = ssdfs_compr_zstd_test_cases,
};

kunit_test_suite(ssdfs_compr_zstd_test_suite);

MODULE_LICENSE("Dual BSD/GPL");
MODULE_AUTHOR("Viacheslav Dubeyko <slava@dubeyko.com>");
MODULE_DESCRIPTION("KUnit tests for SSDFS ZSTD compression");
