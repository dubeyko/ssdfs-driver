 ...
 * @group: PEB group that includes PEB container
 * @group_buf: PEB group buffer
 * @thread_state: PEB container's state
 * @destroy_req: requests to prepare PEB container destroy
...
#ifndef CONFIG_SSDFS_ERASE_BLOCKS_GROUP
	struct ssdfs_peb_group group_buf;
#endif /* CONFIG_SSDFS_ERASE_BLOCKS_GROUP */

	struct ssdfs_thread_state thread_state[SSDFS_PEB_THREAD_TYPE_MAX];
	struct ssdfs_segment_request *destroy_req[SSDFS_PEB_THREAD_TYPE_MAX];
