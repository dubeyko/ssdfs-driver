			     ...
			     u32 free_items)
{
	...
	SSDFS_DBG("old_items_capacity %zu, metadata_items %u, "
		  "used_blks %u, invalid_blks %u, free_items %u, "
		  "new_items_capacity %zu\n",
		  old_items_capacity, blk_bmap->metadata_items,
		  blk_bmap->used_blks, blk_bmap->invalid_blks,
		  free_items, new_items_capacity);
#endif /* CONFIG_SSDFS_DEBUG */

	if (new_items_capacity <= old_items_capacity) {
		new_items_capacity = old_items_capacity;
		new_bmap_bytes = blk_bmap->bytes_count;
