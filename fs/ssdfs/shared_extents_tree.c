...
atomic64_t ssdfs_shextree_folio_leaks;
 ...
 * struct folio *ssdfs_shextree_alloc_folio(gfp_t gfp_mask,
 *                                          unsigned int order)
 * struct folio *ssdfs_shextree_add_batch_folio(struct folio_batch *batch,
 *                                              unsigned int order)
 * void ssdfs_shextree_free_folio(struct folio *folio)
 * void ssdfs_shextree_folio_batch_release(struct folio_batch *batch)
 */
#ifdef CONFIG_SSDFS_MEMORY_LEAKS_ACCOUNTING
...
{
#ifdef CONFIG_SSDFS_MEMORY_LEAKS_ACCOUNTING
	atomic64_set(&ssdfs_shextree_folio_leaks, 0);
...
{
#ifdef CONFIG_SSDFS_MEMORY_LEAKS_ACCOUNTING
	if (atomic64_read(&ssdfs_shextree_folio_leaks) != 0) {
		SSDFS_ERR("SHARED EXTENTS TREE: "
			  "memory leaks include %lld folios\n",
			  atomic64_read(&ssdfs_shextree_folio_leaks));
	}
...
int ssdfs_prepare_shared_extent(struct ssdfs_shared_extent *extent,
				struct ssdfs_btree_search *search)
{
	size_t desc_size = sizeof(struct ssdfs_shared_extent);
...
#endif /* CONFIG_SSDFS_DEBUG */
		search->result.raw_buf.place.ptr = &search->raw.shared_extent;
		search->result.raw_buf.size = desc_size;
		search->result.raw_buf.item_size = desc_size;
...
#ifdef CONFIG_SSDFS_DEBUG
		BUG_ON(!search->result.raw_buf.place.ptr);
		BUG_ON(search->result.raw_buf.size != desc_size);
	...
	ssdfs_memcpy(search->result.raw_buf.place.ptr, 0, desc_size,
		     extent, 0, desc_size,
		     desc_size);
					...
					size_t item_size)
{
	...
	if (found_extents == 1) {
		...
		search->result.raw_buf.item_size = buf_size;
			   ...
			   struct ssdfs_btree_search *search)
{
	...
	if (req_flags & SSDFS_BTREE_SEARCH_INCREMENT_REF_COUNT) {
		...
		if (search->result.raw_buf.size != item_size) {
		...
		ptr = SSDFS_SHARED_EXT(search->result.raw_buf.place.ptr);
				      ...
				      struct ssdfs_btree_search *search)
{
	...
	extent = SSDFS_SHARED_EXT(search->result.raw_buf.place.ptr);
