...
atomic64_t ssdfs_fingerprint_array_page_leaks;
 ...
 * struct page *ssdfs_fingerprint_array_alloc_page(gfp_t gfp_mask)
 * struct page *ssdfs_fingerprint_array_add_pagevec_page(struct pagevec *pvec)
 * void ssdfs_fingerprint_array_free_page(struct page *page)
 * void ssdfs_fingerprint_array_pagevec_release(struct pagevec *pvec)
 */
#ifdef CONFIG_SSDFS_MEMORY_LEAKS_ACCOUNTING
...
{
#ifdef CONFIG_SSDFS_MEMORY_LEAKS_ACCOUNTING
	atomic64_set(&ssdfs_fingerprint_array_page_leaks, 0);
...
{
#ifdef CONFIG_SSDFS_MEMORY_LEAKS_ACCOUNTING
	if (atomic64_read(&ssdfs_fingerprint_array_page_leaks) != 0) {
		SSDFS_ERR("FINGERPRINT ARRAY: "
			  "memory leaks include %lld pages\n",
			  atomic64_read(&ssdfs_fingerprint_array_page_leaks));
	}
